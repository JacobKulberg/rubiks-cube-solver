...
var phase3_table: Dictionary[int, int]
...
func solve(state: RubiksCubeState) -> Array[String]:
	...
	var phase0_turns := search.solve_phase0(s, phase0_table)
	s.apply_turns(" ".join(phase0_turns))

	# Phase G1: Orient all corners and permute M-slice
	var phase1_turns := search.solve_phase1(s, phase1_table)
	s.apply_turns(" ".join(phase1_turns))

	# Phase G2:
	var phase2_turns := search.solve_phase2(s, phase2_table)
	s.apply_turns(" ".join(phase2_turns))

	# Phase G3:
	var phase3_turns := search.solve_phase3(s, phase3_table)
	s.apply_turns(" ".join(phase3_turns))
